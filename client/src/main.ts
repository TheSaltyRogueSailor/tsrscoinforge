...
<h1 style="margin:0; font-size:clamp(32px, 8vw, 40px); line-height:1.05;">TSRS Coin Forge ⚓</h1>
  ...
  <button id="connectWallet" style="background:#2563eb; color:white; border:none; padding:12px 18px; border-radius:12px; font-weight:bold; font-size:16px; cursor:pointer; width:100%; max-width:340px;">      
         ...
         <p id="walletAddress" style="margin:10px 0 0 0; color:#cbd5e1; font-size:14px; font-weight:600; display:inline-block; background:#0f172a; padding:8px 12px; border-radius:999px; border:1px solid #334155;"></p>
        </div>
      </div>
...
<p style="margin:0 0 12px 0; color:#e2e8f0;"><strong>Metadata:</strong> ${launch.metadataUrl ? `<a href="${launch.metadataUrl}" target="_blank" style="color:#38bdf8; text-decoration:none; display:inline-block;">View metadata</a>` : "Not saved yet"}</p>
      ...
      <p style="margin-top:14px; font-size:12px; color:#94a3b8;"><strong>Created:</strong> ${new Date(launch.createdAt).toLocaleString()}</p>
      </div>
